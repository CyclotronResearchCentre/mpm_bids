...
hmri_def.segment.channel.biasfwhm = 40 ; %60;
...
hmri_def.segment.tissue(1).ngaus = 2;
hmri_def.segment.tissue(1).native = [1 1];
hmri_def.segment.tissue(1).warped = [1 1];
hmri_def.segment.tissue(2).tpm = {[hmri_def.TPM ',2']};
hmri_def.segment.tissue(2).ngaus = 2;
hmri_def.segment.tissue(2).native = [1 1];
hmri_def.segment.tissue(2).warped = [1 1];
hmri_def.segment.tissue(3).tpm = {[hmri_def.TPM ',3']};
hmri_def.segment.tissue(3).ngaus = 2;
hmri_def.segment.tissue(3).native = [1 1];
hmri_def.segment.tissue(3).warped = [1 1];
hmri_def.segment.tissue(4).tpm = {[hmri_def.TPM ',4']};
hmri_def.segment.tissue(4).ngaus = 3;
hmri_def.segment.tissue(4).native = [1 1];
hmri_def.segment.tissue(4).warped = [1 1];
hmri_def.segment.tissue(5).tpm = {[hmri_def.TPM ',5']};
hmri_def.segment.tissue(5).ngaus = 4;
hmri_def.segment.tissue(5).native = [1 1];
hmri_def.segment.tissue(5).warped = [1 1];
hmri_def.segment.tissue(6).tpm = {[hmri_def.TPM ',6']};
hmri_def.segment.tissue(6).ngaus = 2;
hmri_def.segment.tissue(6).native = [1 1];
hmri_def.segment.tissue(6).warped = [1 1];
...
hmri_def.segment.warp.samp = 1.2; % old sampling 3 mm. New 1.2 mm double the voxel size  
hmri_def.segment.warp.write = [1 1];
hmri_def.segment.warp.vox   = 0.6;
...
hmri_def.PDproc.biasreg  = 0.0001; % 10^(-5); siya
hmri_def.PDproc.biasfwhm = 40  ;  % reduced the fwhm for high field - siya
hmri_def.PDproc.samp     = 1.2 ;  % reduced the sampling - siya   
...
% ADVANCED USER ONLY.
%--------------------------------------------------------------------------
hmri_def.qMRI_maps_thresh.R1       = 10000 ; % 2000; % 1000*[s-1] siya MPM 7T
...
% 8) 600um protocol MPM travel head:
hmri_def.imperfectSpoilCorr.travelhead.tag = 'travelhead protocol';
hmri_def.imperfectSpoilCorr.travelhead.P2_a = [31.8766,-39.6398,15.0925];
hmri_def.imperfectSpoilCorr.travelhead.P2_b = [-0.066,0.0658,0.9811];
hmri_def.imperfectSpoilCorr.travelhead.small_angle_approx = 1;
% Unknwon protocol
...
hmri_def.MTsatB1CorrectionHelmsC = 0.18; % This value comes from B1 optimiation (see Völzke et al ESMRMB2026)
